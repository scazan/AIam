...
class Application:
    def __init__(self, student, entity, behavior, args):
    ...
    def _update(self):
        if self._now is None:
            ...
            if self._output is not None:
                self._processed_output = self._entity.process_output(self._output)
...
args = parser.parse_args(entity_args_strings)

bvh_reader = BvhReader(SKELETON_DEFINITION)
bvh_reader.read()
pose = bvh_reader.get_hierarchy().create_pose()
entity = Entity(bvh_reader, pose, FLOOR, Z_UP, args)
...
application = Application(student, entity, improvise, args)
application.run()
