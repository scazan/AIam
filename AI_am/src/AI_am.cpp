...
class AIamApp : public AppBasic
{
	...
	private:
		...
		mndl::params::PInterfaceGl mControlParams;

		void drawControl();
		void drawOutput();
		void drawClosingVideo();
...
#define POSE_COUNT ( POSE_MLF + 1 )
		...
		void loadVideo( fs::path videoPath );
...
void AIamApp::prepareSettings( Settings *settings )
{
...
void AIamApp::setup()
{
	...
	mParams = mndl::params::PInterfaceGl( mControlWindow, "Parameters", Vec2i( 250, 450 ), Vec2i( 16, 16 ) );
	...
	mCamera.setAspectRatio( mFbo.getAspectRatio() );
	...
	mParams.addButton( "Go", std::bind( &AIamApp::goToPoseFunc, this ));
	mParams.addSeparator();
	...
	mControlParams = mndl::params::PInterfaceGl( mControlWindow, "Control", Vec2i( 250, 450 ), Vec2i( 282, 16 ) );
	mControlParams.addPersistentSizeAndPosition();
	mFade = 0.f;
	mControlParams.addParam( "Fade", mFade.ptr(), "min=0 max=1 step=.01" );
	mControlParams.addButton( "Fade in", [&]() {
			timeline().apply( &mFade, 1.f, 2.f ); } );
	mControlParams.addButton( "Fade out", [&]() {
			timeline().apply( &mFade, 0.f, 2.f ); } );
	mControlParams.addSeparator();

	vector< string > stateTypeStrs = boost::assign::list_of( "ai dance" )( "closing video" );
	mAppState = STATE_AI;
	...
	mControlParams.addButton( "Load video", std::bind( &AIamApp::loadVideo, this, fs::path() ) );
	mControlParams.addPersistentParam( "Video filename", &mVideoFileName, "", "", true );
	if ( mVideoFileName != "" )
		loadVideo( mVideoFileName );
}

void AIamApp::loadVideo( fs::path videoPath )
{
	if ( videoPath.empty() )
	{
...
#endif
		videoPath = ci::app::getOpenFilePath( appPath );
	}
	...
	if ( !videoPath.empty() )
	{
		try
		{
			mMovie = qtime::MovieGl( videoPath );
			mVideoFileName = videoPath.string();
		}
		catch ( const qtime::QuickTimeExc &exc )
		{
			app::console() << "Could not load " << videoPath.string() << endl;
		}
	}
}

void AIamApp::goToPoseFunc()
{
...
vector< fs::path > AIamApp::getMotions( const fs::path &relativeDir )
{
...
void AIamApp::draw()
{
	...
	if ( mAppState == STATE_AI )
		drawOutput();
	else if ( mAppState == STATE_CLOSING_VIDEO )
		drawClosingVideo();
}

void AIamApp::drawControl()
{
	gl::clear();
...
void AIamApp::drawOutput()
{
	...
	if ( !mAllMotionsLoaded )
	{
		...
		float w = mFbo.getWidth();
		float h = mFbo.getHeight();
		float h2 = h / 2.f;
		float s = h / 50.f;
	...
	if ( !mAllMotionsLoaded )
		...
		gl::color( Color::gray( mFade ) );
	gl::draw( mFbo.getTexture(), outputRect );
}

void AIamApp::drawClosingVideo()
{
	gl::clear();
	gl::setViewport( getWindowBounds() );
	gl::setMatricesWindow( getWindowSize() );
	...
	{
		gl::Texture movieTex = mMovie.getTexture();

		if ( movieTex )
		{
			Rectf centeredRect = Rectf( movieTex.getBounds() ).getCenteredFit(
					getWindowBounds(), true );

			gl::color( Color::gray( mFade ) );
			gl::draw( movieTex, centeredRect );
		}
    }
}

bool AIamApp::positionReceived( const mndl::osc::Message &message )
{
	// /position s s f
...
void AIamApp::mouseDown( MouseEvent event )
{
...
void AIamApp::mouseDrag( MouseEvent event )
{
	if ( getWindow() == mOutputWindow )
	{
		mMayaCam.setCurrentCam( mCamera );
...
TriMesh AIamApp::createSquare( const Vec2i &resolution )
{
...
void AIamApp::keyDown( KeyEvent event )
{
...
CINDER_APP_BASIC( AIamApp, RendererGl )
