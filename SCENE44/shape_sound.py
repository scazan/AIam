...
class UserMovementInterpreter:
    ...
    def set_up_osc_receiver(self):
        ...
        osc_receiver.add_method("/center", "ifff", self._handle_center)
        osc_receiver.start()
    ...
    def _reset(self):
        ...
        self.nearest_point_on_shape = None
    ...
    def _process_frame(self):
        ...
        if args.enable_features and self._selected_user is not None:
            self._frame["features"] = self._extract_features()
            self._output_controller.send_features(self._frame["features"])
        ...
        if args.with_viewer:
            self._frame["selected_user"] = self._selected_user
    ...
    def _get_nearest_point_on_shape(self, reference):
        return min(shape, key=lambda point_on_shape: self._get_distance(point_on_shape, reference))
...
class SceneWithShape(TrackedUsersScene):
    def render_3d_scene(self):
        TrackedUsersScene.render_3d_scene(self)
        self._render_shape()
        self._render_center_of_mass()
        self._render_nearest_point_on_shape()
    ...
    def _render_center_of_mass(self):
        if self.parent().interpreter.center_of_mass is not None:
            x, z = self.parent().interpreter.center_of_mass
            glColor3f(0, 0, 1)
            glPointSize(5)
            glBegin(GL_POINTS)
            glVertex3f(x, self.parent().floor_y, z)
            glEnd()

    def _render_nearest_point_on_shape(self):
        if self.parent().interpreter.nearest_point_on_shape is not None:
            x, z = self.parent().interpreter.nearest_point_on_shape
            glColor3f(0, 1, 1)
            glPointSize(5)
            glBegin(GL_POINTS)
            glVertex3f(x, self.parent().floor_y, z)
