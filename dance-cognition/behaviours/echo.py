...
class Behaviour(behaviour.Behaviour):
    ...
    def _state_observed(self, new_observed_state):
        if self._observed_state and self._is_valid_transition(self._observed_state, new_observed_state):
            self._transition_observed(self._observed_state, new_observed_state)
    ...
    def _transition_observed(self, source_state, destination_state):
        self._output_queue.append((source_state, destination_state))

    def output(self):
        if self._output_transition:
            source_state, destination_state = self._output_transition
            return InterStatePosition(
                source_state, destination_state,
                self._output_transition_time / self._output_transition_duration)
